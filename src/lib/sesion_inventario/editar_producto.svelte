<script>
  ...
  import { obtenerProductoPorId, actualizarProducto, eliminarProductoPorId } from '$lib/conexion_products';
  ...
  function limpiarNumeros(producto) {
    if (producto.col_precio_compra)
      producto.col_precio_compra = parseFloat(producto.col_precio_compra.toString().replace(/[^0-9.-]+/g, ""));
    if (producto.col_precio_puerta)
      producto.col_precio_puerta = parseFloat(producto.col_precio_puerta.toString().replace(/[^0-9.-]+/g, ""));
    if (producto.col_precio_domicilio)
      producto.col_precio_domicilio = parseFloat(producto.col_precio_domicilio.toString().replace(/[^0-9.-]+/g, ""));
    if (producto.col_precio_puerta11)
      producto.col_precio_puerta11 = parseFloat(producto.col_precio_puerta11.toString().replace(/[^0-9.-]+/g, ""));
    if (producto.col_precio_domicilio11)
      producto.col_precio_domicilio11 = parseFloat(producto.col_precio_domicilio11.toString().replace(/[^0-9.-]+/g, ""));
...
<div class="popup">
  <div class="popup-header">
    ...
    <div class="popup-actions popup-actions-row">
      {#if producto.id}
        <button type="button" class="btn eliminar icon-action" on:click={handleEliminarProducto} disabled={cargando} title="procede con precaución">
          <div class="icon-img bordered red">
            <svg width="50" height="50" viewBox="0 0 50 50" fill="none" aria-hidden="true" focusable="false">
              <rect x="12" y="18" width="26" height="22" rx="3" fill="#e53935" stroke="#b71c1c" stroke-width="2"/>
              <rect x="19" y="12" width="12" height="6" rx="2" fill="#e53935" stroke="#b71c1c" stroke-width="2"/>
              <rect x="16" y="10" width="18" height="4" rx="2" fill="#b71c1c"/>
              <line x1="20" y1="23" x2="20" y2="36" stroke="#fff" stroke-width="2.2" stroke-linecap="round"/>
              <line x1="25" y1="23" x2="25" y2="36" stroke="#fff" stroke-width="2.2" stroke-linecap="round"/>
              <line x1="30" y1="23" x2="30" y2="36" stroke="#fff" stroke-width="2.2" stroke-linecap="round"/>
            </svg>
          </div>
      ...
      <button type="button" class="guardar-png-btn" on:click={guardarProducto} disabled={cargando} aria-label="Guardar">
  {#if cargando}
...
.guardar-label {
  ...
  <button type="button" class="cerrar-btn" on:click={() => dispatch('cancelar')} disabled={cargando} aria-label="Cerrar">
